...
void init_rect(struct rect *rec)
{
  ...
  rec->width = 0.0;
  rec->theta = 0.0;
  rec->p = 0.0; rec->prec = 0.0; char* imageName = "convertPGM.pgm";
  cv::Mat image;
...
int main(int argc, char **argv)
{
  ...
  int ell_count = 0, line_count = 0, circ_count = 0;
   ...
   cv::imwrite("convertPGM.pgm", image_import);
...
//
  ...
  image = read_pgm_image_double("convertPGM.pgm");

  EllipseDetection(image, rho, prec, p, eps, smooth, &ell_count, &circ_count,
                   &line_count,"convertPGM.pgm");
  printf("%s\n", "convertPGM.pgm");
