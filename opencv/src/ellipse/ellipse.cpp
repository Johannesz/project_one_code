...
//	for (y = 0; y < ysize; y++)
//		for (x = 0; x < xsize; x++)
//			image->data[x + y * xsize] =
//					bin ? (double) getc(f) : (double) get_num(f);
//
...
//			err = term
//					* ((1.0 - pow(mult_term, (double) (n - i + 1)))
//							/ (1.0 - mult_term) - 1.0);
//
...
//		struct coorlist **list_p, void **mem_p, image_double *gradx,
//		image_double *grady, image_double *grad, unsigned int n_bins,
//		double max_grad) {
//	/* check parameters */
//	if (in == NULL || in->data == NULL || in->xsize <= 0 || in->ysize <= 0)
...
//		double eps, int smooth, int *ell_count, int *circ_count,
//		int *line_count, char *fstr) {
...
//		angles = ll_angle(imgauss, rho, &list_p, &mem_p, &gradx, &grady, &grad,
//				n_bins, max_grad);
//	} else
//		angles = ll_angle(image, rho, &list_p, &mem_p, &gradx, &grady, &grad,
//				n_bins, max_grad);
//
//	xsize = angles->xsize;
//	ysize = angles->ysize;
...
//	logNT[2] = 4.0 * (log10((double) xsize) + log10((double) ysize))
//			+ log10(9.0) + log10(3.0); /* N^8 */
//	/* number of tests for circular arcs */
//	logNT[1] = 3.0 * (log10((double) xsize) + log10((double) ysize))
//			+ log10(9.0) + log10(3.0); /* N^6 */
//	/* number of tests for line-segments */
//	logNT[0] = 5.0 * (log10((double) xsize) + log10((double) ysize)) / 2.0
//			+ log10(11) + log10(3.0); /* N^5 */
//
...
//			valid_line(reg, &reg_size, reg_angle, prec, p, &rec, lin, grad,
//					gradx, grady, used, angles, density_th, logNT[0], mlog10eps,
//					&nfa[0]);
...
//				valid_curve(reg, &reg_size, prec, p, angles, used, grad, gradx,
//						grady, paramc, parame, rec, logNT, mlog10eps,
//						density_th, min_size, nfa, pext, regc, rege, regp_size);
//
...
//					for (i = 0; i < regp_size[0]; i++)
//						used->data[regl[i].y * used->xsize + regl[i].x] =
//								NOTUSED;
//					for (i = 0; i < regp_size[1]; i++)
//						used->data[regc[i].y * used->xsize + regc[i].x] =
//								NOTUSED;
//					for (i = 0; i < regp_size[2]; i++)
//						if (rege[i].z == USEDELL) {
//							used->data[rege[i].y * used->xsize + rege[i].x] =
//									USED;
//						} else {
//							used->data[rege[i].y * used->xsize + rege[i].x] =
//									USEDELLNA;
//						}
//				} else if (nfa[1] > mlog10eps && nfa[1] > nfa[0]
//						&& nfa[1] > nfa[2] && regp_size[1] > min_size[1]) /* circle */
//						{
//					(*circ_count)++;
//					/*if (smooth)
...
//							used->data[regc[i].y * used->xsize + regc[i].x] =
//									USEDCIRCNA;
//				} else if (nfa[0] > mlog10eps && regp_size[0] > min_size[0]
//						&& nfa[0] > nfa[1] && nfa[0] > nfa[2]) /* line */
//						{
//					(*line_count)++;
...
//	cv::Mat image_import;
//	image_import = imread(imageName, cv::IMREAD_GRAYSCALE);
...
//	for (y = 0; y < ysize; y++){
//		for (x = 0; x < xsize; x++){
//			image->data[x + y * xsize] = image_import.at<char>(x,y);
//
//		}
//	}
//
...
//    //cv::imwrite("convertPGM.pgm", image, compression_params);
//
//    std::cout << "PGM: " << image << std::endl << std::endl;
//
//    cv::imwrite("convertPGM.pgm", image);
//
//
//
//
